...
echo "🔧 Testing CANCEL_ORDER pattern recognition..."
echo "Query: 'cancel order ORDER-2024-001'"
curl -X POST http://localhost:8093/v1/request \
  -H "Content-Type: application/json" \
  -H "X-User-ID: test-user" \
  -H "Authorization: Bearer test-token" \
  -d '{"query": "cancel order ORDER-2024-001", "environment": "dev"}' \
  | grep -E '"task_id"|"confidence"|"order_id"|"service"'
echo ""
echo ""

# Test CHANGE_ORDER_STATUS operation
echo "🔧 Testing CHANGE_ORDER_STATUS pattern recognition..."
echo "Query: 'change order status to completed'"
curl -X POST http://localhost:8093/v1/request \
  -H "Content-Type: application/json" \
  -H "X-User-ID: test-user" \
  -H "Authorization: Bearer test-token" \
  -d '{"query": "change order status to completed for ORDER-456", "environment": "prod"}' \
  | grep -E '"task_id"|"confidence"|"target_status"|"order_id"'
echo ""
echo ""
...
echo "   • Entity extraction successfully identifies order IDs and target statuses"
